...
const Tipo = db.tipo_movimientos;
...
module.exports = {
    ...
    list(req, res) {
        ...
        Tipo.findAndCountAll({ where: condition,order:[[`${criterio}`,`${order}`]],limit,offset})
        .then(data => {
    ...
    get (req, res) {
        Tipo.findAll({attributes: ['id', 'nombre']})
        .then(data => {
            res.send(data);
        })
        .catch(error => {
