...
module.exports = {
    ...
    upload(req, res) {
        res.status(200).jsonp(req.file.filename)          
    },
    ...
    getDocument (req, res) {
        ...
        fs.readFile(`./documents/` + nombre, 'utf8', (err, data) => {
            if (err) throw err;
            res.send(data)
        })
    },
    ...
    list(req, res) {
        ...
        var condition = busqueda ? { [Op.or]: [{ contenido: { [Op.like]: `%${busqueda}%` } }], id_paciente: patientId  } : null ;

        Documento.findAndCountAll({
            include: [
                {
                    model: Usuario,
                    require: true
